...
const Spinner: React.FC = () => (
  <div className="py-[48px] px-[20px] text-center text-[14px] text-[#94a3b8]">
    <div
      className={`
        mx-auto mb-[12px] w-[28px] h-[28px]
        rounded-full border-[2.5px]
        border-[rgba(99,179,237,0.22)] border-t-[#38bdf8]
        animate-spin
      `}
    />
    Đang tải dữ liệu…
...
const EmptyState: React.FC<{ message: string; isError?: boolean }> = ({ message, isError }) => (
  <div
    className={`
      py-[48px] px-[20px] text-center text-[14px]
      ${isError ? 'text-[#f87171]' : 'text-[#94a3b8]'}
    `}
  >
    {isError && '⚠ '}
    {message}
...
const EventThumbnail: React.FC<{ bannerUrl?: string; name: string }> = ({ bannerUrl, name }) => (
  <div
    className={`
      w-[72px] h-[50px] shrink-0 overflow-hidden
      rounded-[8px] bg-[#1a2235]
...
const ProgressBar: React.FC<{ sold: number; maxSupply: number }> = ({ sold, maxSupply }) => {
  ...
  return (
    <div
      className="mt-[5px] h-[4px] w-full min-w-[100px] overflow-hidden rounded-[4px] bg-[#1a2235]"
      title={`${pct}% đã bán`}
    >
      <div
        className="h-full rounded-[4px] bg-[#34d399] transition-all duration-300"
        style={{ width: `${pct}%` }}
      />
    </div>
  );
...
/* ── Single event row ── */
const EventRow: React.FC<{ event: OrganizerEvent }> = ({ event }) => (
  <article
    className={`
      grid grid-cols-[1fr_auto] sm:grid-cols-[72px_1fr_auto]
...
/* ══════════════════════════════════════════
   Main component
   ══════════════════════════════════════════ */
const OrganizerEventList: React.FC<OrganizerEventListProps> = ({ events, loading, error }) => {
