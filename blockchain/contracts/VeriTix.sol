...
contract VeriTix is ERC721URIStorage, Ownable, ReentrancyGuard {
    ...
    function buyTickets(uint256 eventId, uint256 quantity, string[] calldata _tokenURIs) public payable nonReentrant {
        Event storage myEvent = events[eventId];
        require(myEvent.isActive, "Su kien da dong");
        require(quantity > 0 && quantity <= 3, "So luong tu 1 den 3");
        require(myEvent.currentMinted + quantity <= myEvent.maxSupply, "Het ve");
        require(msg.value >= myEvent.price * quantity, "Khong du tien");
        require(_tokenURIs.length == quantity, "Thieu TokenURI");
        ...
        myEvent.currentMinted += quantity;
    }
